...
// 产生选项（过滤掉不支持的选项）
function generateOptions(len, disabledOptions, hideDisabledOptions, step = 1) {
  const arr = [];
  // 循环 value 并每次增加一个区间
...
// 获取最近的有效时间(time和每一个时间求绝对值，然后排序)
// 获取绝对值最小的时分秒，然后再moment转换
function toNearestValidTime(time, hourOptions, minuteOptions, secondOptions) {
  const hour = hourOptions.slice().sort((a, b) => {
    return Math.abs(time.hour() - a) - Math.abs(time.hour() - b);
  })[0];
  const minute = minuteOptions.slice().sort((a, b) => {
    return Math.abs(time.minute() - a) - Math.abs(time.minute() - b);
  })[0];
  const second = secondOptions.slice().sort((a, b) => {
  ...
  disabledHours = () => {
    ...
    if (use12Hours && Array.isArray(disabledOptions)) {
      if (this.isAM()) {
        ...
        disabledOptions = disabledOptions.amp(h => {
          return h === 12 ? 12 : h - 12;
        });
      }
    }
